...
public class Main {
    ...
    private static void run(DNP3Manager manager, String outStIP, int port) throws DNP3Exception, IOException {
        ...
        //masterConfig.timeSyncMode = TimeSyncMode.LAN;
        ...
        Master master1 = channel1.addMaster("master", MyPrintingSOEHandler.getInstance(), DefaultMasterApplication.getInstance(), config1);

        // do an integrity scan every 2 seconds
        byte variations = 3;
        byte group = 30;
        //master1.addPeriodicScan(Duration.ofSeconds(10),Header.allObjects(group,variations),MyPrintingSOEHandler.getInstance());
        ...
        while (true) {
            ...
            switch (line) {
                case ("quit"):
                    return;
                case "1":{
                    master1.scan(Header.allObjects(group,variations),MyPrintingSOEHandler.getInstance());
                    break;
                }
                case "2":{
                    MyPrintingSOEHandler.getInstance();
                    break;
                }
                case "3":{
                    master1.scan(Header.getIntegrity(),MyPrintingSOEHandler.getInstance());
                    break;
                }
                case ("crob"):
